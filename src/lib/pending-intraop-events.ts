...
type DroppedIntraopEvent = {
  caseId: string
  event: Omit<PendingIntraopEvent, "syncStatus">
  status: number
  droppedAt: string
}
...
async function loadPending(caseId: string): Promise<PendingIntraopEvent[]> {
...
async function storePending(caseId: string, events: PendingIntraopEvent[]) {
  if (events.length === 0) {
...
async function recordDroppedEvent(caseId: string, ev: PendingIntraopEvent, status: number): Promise<void> {
  try {
...
export async function getDroppedIntraopEvents(): Promise<DroppedIntraopEvent[]> {
  const raw = await SecureStore.getItemAsync(DROPPED_KEY)
  if (!raw) return []
