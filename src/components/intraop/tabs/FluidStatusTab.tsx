 ...
 *
 * Everything here except blood loss is a projection of the timetable — the same
 * numbers the web record shows, computed by the same Core functions, so the two
 * cannot drift. Blood loss is the one figure only the anaesthetist can state,
 * and it is the only editable control on the tab.
 */
export function FluidStatusTab({
  infusionTotals, bolusTotals, weightNote,
  crystalloidsMl, colloidsMl, bloodMl, urineMl,
  ...
  // The figure is persisted when the tab is left, the same way premedication
  // is, so there is no save control here.
  setBloodLossMl: (value: number | null) => void
  tc: (key: ClinicalStringKey) => string
}) {
  const balance: { label: string; value: number | null; color: string }[] = [
    ...
    { label: tc("urineOutputLabel"),  value: urineMl,        color: "#fbbf24" },
  ]
    ...
    <ScrollView style={{ flex:1 }} contentContainerStyle={{ padding:16, paddingBottom:40 }}>
      {infusionTotals.length > 0 && (
        <View style={{ marginBottom:24 }}>
          ...
          {infusionTotals.map(row => (
            <TotalRow key={`${row.name}-${row.unit}`} name={row.name} total={row.total} unit={row.unit}
              marked={row.weightUsed != null} />
          ))}
          {weightNote && (
      ...
      <View style={{ marginBottom:24 }}>
        ...
        <View style={{ flexDirection:"row", flexWrap:"wrap", gap:8 }}>
          {balance.map(({ label, value, color }) => (
            <View key={label} style={{
              flexGrow:1, flexBasis:"45%", paddingVertical:10, paddingHorizontal:12, borderRadius:10,
...
function TotalRow({ name, total, unit, marked = false }: {
  name: string
  total: number
  unit: string
  marked?: boolean
  ...
  return (
    <View style={{
      ...
      <Text style={{ color:"#cbd5e1", fontSize:13, fontWeight:"600", flex:1 }} numberOfLines={1}>{name}</Text>
