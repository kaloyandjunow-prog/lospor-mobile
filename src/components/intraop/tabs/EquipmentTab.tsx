      ...
      {medicationWarnings.length > 0 && (
        <View style={{ backgroundColor:shade("#2a1410"), borderRadius:14, borderWidth:1, borderColor:shade("#7c2d12"),
          ...
          {medicationWarnings.map(w => (
            <Text key={w.key} style={{ color:shade("#fb923c"), fontSize:13, fontWeight:"700" }}>⚠ {w.label}</Text>
          ))}
        </View>
      )}
      {!hasPreop ? (
        <View style={{ backgroundColor:shade("#111111"), borderRadius:14, borderWidth:1, borderColor:shade("#1e2d40"),
          padding:20, alignItems:"center" }}>
          <Text style={{ color:shade("#64748b"), fontSize:13, textAlign:"center", lineHeight:20 }}>
        ...
        <>
          ...
          <View style={{ backgroundColor:shade("#111111"), borderRadius:14, borderWidth:1, borderColor:shade("#1e2d40"),
            ...
            {preop?.age    != null && <Text style={{ color:shade("#94a3b8"), fontSize:13 }}>{tc("agePlaceholder")} <Text style={{ color:shade("#f8fafc"), fontWeight:"700" }}>{preop.age < 1 ? `${Math.round(preop.age * 12)}${tc("monthsShort")}` : `${preop.age}${tc("yearsShort")}`}</Text></Text>}
            {preop?.weight != null && <Text style={{ color:shade("#94a3b8"), fontSize:13 }}>{tc("weightPlaceholder")} <Text style={{ color:shade("#f8fafc"), fontWeight:"700" }}>{preop.weight} kg</Text></Text>}
            {preop?.height != null && <Text style={{ color:shade("#94a3b8"), fontSize:13 }}>{tc("heightPlaceholder")} <Text style={{ color:shade("#f8fafc"), fontWeight:"700" }}>{preop.height} cm</Text></Text>}
            {preop?.sex    != null && <Text style={{ color:shade("#94a3b8"), fontSize:13 }}>{tc("sexLabel")} <Text style={{ color:shade("#f8fafc"), fontWeight:"700" }}>{preop.sex === "MALE" ? tc("sexMale") : preop.sex === "FEMALE" ? tc("sexFemale") : preop.sex}</Text></Text>}
          </View>
          ...
          {cats.map((cat, ci) => (
            <View key={cat.cat} style={{ marginBottom: ci < cats.length - 1 ? 16 : 0 }}>
              ...
              <View style={{ backgroundColor:shade("#111111"), borderRadius:14, borderWidth:1, borderColor:shade("#1e2d40"), padding:14 }}>
                ...
                {cat.items.map((item, ii) => (
                  <View key={item.label} style={{
                    ...
                    borderBottomColor:shade("#1e2d40"),
                  }}>
                    <View style={{ flexDirection:"row", alignItems:"flex-start", gap:12 }}>
                      <Text style={{ color:shade("#64748b"), fontSize:13, flexShrink:1 }}>{item.label}</Text>
                      <Text style={{ color:shade("#f8fafc"), fontSize:13, fontWeight:"700", flex:1, minWidth:0, textAlign:"right" }}>
                        {item.value}
                      </Text>
                    </View>
                    {!!item.note && (
                      <Text style={{ color:shade("#475569"), fontSize:11, lineHeight:15 }}>{item.note}</Text>
                    )}
                  </View>
                ))}
              </View>
