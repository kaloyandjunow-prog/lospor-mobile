...
export function SegmentButton({ label, selected, onPress, flex = 1, disabled = false }: {
  label: string
  selected: boolean
  onPress: () => void
  flex?: number
    ...
    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
      {bloodGroupOptions.map(opt => {
        ...
        return (
          ...
          >
            <Text style={{ color: selected ? colors.primary : colors.textSecondary, fontSize: 15, fontWeight: "900" }}>{displayOption("BLOOD_GROUP", opt, language)}</Text>
          </Pressable>
        )
      })}
    </View>
...
export { ManualLabPanel } from "@/components/preop/ManualLabPanel"
